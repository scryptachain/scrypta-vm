...
async function publishModule() {
    if (argv.m !== undefined && argv.i !== undefined) {
        ...
        if (name !== undefined && author !== undefined && version !== undefined) {
            ...
            let module = new CoinKey(Buffer.from(hash, 'hex'), {
                private: 0xae,
                public: 0x30,
                scripthash: 0x0d
            })

            console.log('MODULE ADDRESS IS: ' + module.publicAddress)
            manifest.address = module.publicAddress
            let sid = await scrypta.buildWallet('TEMPORARY', module.publicAddress, { prv: module.privateWif, key: module.publicKey.toString('hex') }, false)
            ...
            if (balance.balance > 0.011) {
                ...
                if (genesis_check.data[0] !== undefined) {
                    ...
                    genesis.module = JSON.parse(genesis.message)
                    if (genesis.module.name === manifest.name && genesis.module.address === manifest.address) {
                        ...
                        if (version_check.data[0] === undefined && genesis.module.version !== manifest.version) {
                            console.log('PUBLISHING UPDATE')
                            let signed = await scrypta.signMessage(identity, JSON.stringify(manifest))
                            let moduleBalance = await scrypta.get('/balance/' + manifest.address)
                            let funded = false
                            if (moduleBalance.balance < 0.002) {
                            ...
                            console.log('VERSION EXIST, PLEASE UPDATE MODULE FIRST')
                        }
                    } else {
                ...
                console.log('MODULE DOESN\'T HAVE REQUIRED BALANCE: ' + balance.balance)
            }
        } else {
            console.log('CAN\'T FIND MANIFEST')
        }
    } else {
        console.log('PLEASE PROVIDE MODULE PATH AND IDENTITY PRIVATE KEY')
    }
}

async function fundAddress(moduleAddress) {
    return new Promise(async response => {
        ...
        while (funded === false) {
            ...
            let sent = await scrypta.send(sid, 'TEMPORARY', moduleAddress, 0.01)
